...
{
    public partial class DangKyCapPhepXD : Form
    {
        int AddQuan = 0;
        ...
        public DangKyCapPhepXD(string Day, string month, string year)
        {
            InitializeComponent();
            textBox4.Text = Day;
            textBox3.Text = month;
        ...
        private void DangKyCapPhepXD_Load(object sender, EventArgs e)
        {
            ...
            if (IDChinhSua != 0)
            {
                ...
                SoSeri = ds3.Tables[0].Rows[0]["seriGCN"].ToString();
                NgayCapGiayCN = DateTime.Parse(ds3.Tables[0].Rows[0]["NgayCapGCN"].ToString());
                txtGhiChu.Text = SoSeri = ds3.Tables[0].Rows[0]["GhiChu"].ToString(); 
                GiayToLienQuan.Text = "Giấy chứng nhận QSDĐ số seri " + ds3.Tables[0].Rows[0]["seriGCN"].ToString() + " do Sở Tài nguyên & Môi trường TP Đà Nẵng cấp ngày " + NgayCapGiayCN.Day + "/" + NgayCapGiayCN.Month + "/" + NgayCapGiayCN.Year + ".";
                //NoiDung.Text = qdkhac;
        ...
        string ID;
        public DangKyCapPhepXD(string ID1,string cdt, string diachi, string congtrinh, string thietke, string soto, string sothua, string duong, string viahe, string khuqh,string quan, string phuong, string dientich, string dientichxaydung, string tongdtsd, string mdxd, string hesosdd, string chieucaoct, string sotang, string chieucaocactang, string cotnenct, string khoanglui, string dovuon, string serigcn, string ngaycapgcn, string qdkhac,string ghichu)
        {
            InitializeComponent();
            ...
            try
            {
                if (LoaiHS == 2)
                {
                    if (STo.Text != "" | SThua.Text != "" | Phuong.Text != "")
                    {
                        ...
                        string querystr2 = "UPDATE CPXD SET [TinhTrang] =N'Đang trình phê duyệt' FROM CPXD WHERE [SoTo] =" + STo.Text + " and [SoThua] =" + SThua.Text + " and [IDPhuong] = N'" + IDPhuong + "'";
                ...
                {
                    string cmdString = "INSERT INTO CPXD ([TenHS] ,[TenCDT],[DiaChi],[SoThua],[SoTo],[DT],[DTXD],[LoaiCT],[ThietKe],[ChieuCao],[SoTang],[TinhTrang],[SoQD],[IDPhuong],[Duong],[ViaHe] ,[MDXD],[HESOSDD],[ChieuCaoCacTang],[CotNen],[KhoangLui],[DoVuon],[seriGCN],[TongDTSDD],[FileQD],[NgayCapGCN],[NgayThemHS]) VALUES (@TenHS, @TenCDT, @DiaChi,@SoThua, @SoTo, @DT,@DTXD, @LoaiCT,@ThietKe, @ChieuCao, @SoTang,  @TinhTrang, @SoQD, @IDPhuong,@Duong, @ViaHe, @MDXD  , @HESOSDD,  @ChieuCaoCacTang, @CotNen, @KhoangLui,@DoVuon, @seriGCN, @TongDTSDD   ,@FileQD, @NgayCapGCN, @NgayThemHS)";
                    ...
                    using (SqlConnection conn = new SqlConnection(connString))
                    {
                        using (SqlCommand comm = new SqlCommand())
                        {
                            ...
                            comm.Parameters.AddWithValue("@seriGCN", SoSeri);
                            comm.Parameters.AddWithValue("@TongDTSDD", TongDTSD.Text);
                            comm.Parameters.AddWithValue("@FileQD", "");
                            comm.Parameters.AddWithValue("@NgayCapGCN", NgayCapGiayCN.ToString("yyyy-MM-ddTHH:mm:ss.fff"));
                            ...
                            DialogResult dialogResult = MessageBox.Show("Chưa có dữ liệu đính kèm hồ sơ cấp phép, Có thêm dữ liệu hồ sơ cấp phép không ?", "Thông báo", MessageBoxButtons.YesNo);
                            if (dialogResult == DialogResult.Yes)
                            {
                                //lấy mã hs
                                string sql = "select ID from CPXD where ID = (SELECT max(ID) FROM CPXD where [SoTo] ='" + STo.Text + "' and [SoThua] ='" + SThua.Text + "')";
                                SqlConnection connection = new SqlConnection(QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
                                SqlCommand commanCPXD = new SqlCommand(sql, connection);
                                commanCPXD.Connection.Open();
                                string pathMaHS = "";
                                ...
                                if (openFileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                                {
                                    ...
                                    {
                                        //System.IO.File.Copy(openFileDialog.FileName, Properties.Settings.Default.PathData + "\\FileDinhKemCPXD\\" + System.IO.Path.GetFileName(openFileDialog.FileName));
                                        using (WebClient client = new WebClient())
                                        {
                                            client.Credentials = new NetworkCredential("sinhnv", "Abc#1234");
                                            client.UploadFile("ftp://117.2.120.9/FileDinhKemCPXD/" + pathMaHS.Replace(" ", "").ToString() + "/" + System.IO.Path.GetFileName(openFileDialog.FileName), WebRequestMethods.Ftp.UploadFile, openFileDialog.FileName);
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            //System.IO.File.Copy(openFileDialog.FileNames[i], Properties.Settings.Default.PathData + "\\FileDinhKemCPXD\\" + System.IO.Path.GetFileName(openFileDialog.FileNames[i]));
                                            using (WebClient client = new WebClient())
                                            {
                                                client.Credentials = new NetworkCredential("sinhnv", "Abc#1234");
                                                client.UploadFile("ftp://117.2.120.9/FileDinhKemCPXD/" + pathMaHS.Replace(" ", "").ToString() + "/" + System.IO.Path.GetFileName(openFileDialog.FileNames[i]), WebRequestMethods.Ftp.UploadFile, openFileDialog.FileNames[i]);
                                            }
                                        }
                                    }
                                }
