...
{
    public partial class QuanlyCapPhepXD : Form
    {
        ...
        string sql1 = "SELECT *,(select Xa.TenPhuong from PhuongXa Xa where Xa.MaPhuong = IDPhuong) as TenPhuong,(select QH.TENHUYEN from Quan QH where QH.MAHUYEN = IDHuyen) as QuanHuyen FROM CPXD where ID = '" + ID1 + "'";
        public QuanlyCapPhepXD()
        {
            InitializeComponent();
        ...
        private void QuanlyCapPhepXD_Load(object sender, EventArgs e)
        {
        ...
        void showgridControl1()
        {
            ...
            string sql = "select * from CPXD order by NgayThemHS DESC";
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            ThemHSCapPhep frm = new ThemHSCapPhep(date.Day.ToString(), date.Month.ToString(), date.Year.ToString());
            frm.ShowDialog();
            showgridControl1();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                QLHTDT.FormPhu.QLQH.listfilefolder frm = new QLHTDT.FormPhu.QLQH.listfilefolder(path);
                frm.Show();
            }
            else
            {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            string sothuaqr = GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "SoThua").ToString();
            ...
            if (openFileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                if (frmCapNhatSoGP.Visible == false && CapNhatSoGPXD.SoGP != "")
                {
                    try
                    {
                        ...
                        string querystr = "UPDATE CPXD SET [SoQD] ='" + CapNhatSoGPXD.SoGP + "' FROM CPXD WHERE [SoTo] =" + sotoqr + " and [SoThua] =" + sothuaqr + " and [IDPhuong] = N'" + Phuongqr + "' AND ID = (SELECT max(ID) FROM CPXD where [SoTo] ='" + sotoqr + "' and [SoThua] ='" + sothuaqr + "' and [IDPhuong] = N'" + Phuongqr + "')";
                        SqlCommand query = new SqlCommand(querystr, connection);
                        query.ExecuteNonQuery();
                        string querystr2 = "UPDATE CPXD SET [TinhTrang] =N'Đã có kết quả' FROM CPXD WHERE [SoTo] =" + sotoqr + " and [SoThua] =" + sothuaqr + " and [IDPhuong] = N'" + Phuongqr + "' AND ID = (SELECT max(ID) FROM CPXD where [SoTo] ='" + sotoqr + "' and [SoThua] ='" + sothuaqr + "' and [IDPhuong] = N'" + Phuongqr + "')";
                        SqlCommand query2 = new SqlCommand(querystr2, connection);
                        query2.ExecuteNonQuery();
                        string querystr3 = "UPDATE CPXD SET [NgayCap] = '" + CapNhatSoGPXD.Nam + "-" + CapNhatSoGPXD.Thang + "-" + CapNhatSoGPXD.Ngay + "' FROM CPXD WHERE [SoTo] =" + sotoqr + " and [SoThua] =" + sothuaqr + " and [IDPhuong] = N'" + Phuongqr + "' AND ID = (SELECT max(ID) FROM CPXD where [SoTo] ='" + sotoqr + "' and [SoThua] ='" + sothuaqr + "' and [IDPhuong] = N'" + Phuongqr + "')";
                        SqlCommand query3 = new SqlCommand(querystr3, connection);
                        query3.ExecuteNonQuery();
                        string folder = Properties.Settings.Default.PathData + "\\FileDinhKemCPXD\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString().Replace(" ", "").ToString();
                        if (!Directory.Exists(folder))
                        {
                            Directory.CreateDirectory(folder);
                        }
                        string file = Properties.Settings.Default.PathData + "\\FileDinhKemCPXD\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString().Replace(" ", "").ToString() + "\\GPXD" + CapNhatSoGPXD.SoGP + ".pdf";
                        showgridControl1();
                        MessageBox.Show("Cập nhật quyết định thành công","Thông báo");
                    }
                    catch (Exception ex)
        ...
        private void ViTriTDat(bool ThongTinThua)
        {
            try
            {
                ...
                if (layeredit != null)
                {
                    ...
                    else if (GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "TenCDT").ToString() != "")
                    {
        ...
        private void GridView1_Click_1(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                try
                {
                    string Caption = GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "TenCDT").ToString();
                    cardView1.CardCaptionFormat = Caption;
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString(), out ID1);
                    sql1 = "SELECT *,(select Xa.TenPhuong from PhuongXa Xa where Xa.MaPhuong = IDPhuong) as TenPhuong,(select QH.TENHUYEN from Quan QH where QH.MAHUYEN = IDHuyen) as QuanHuyen FROM CPXD where ID = '" + ID1 + "'";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox1.Checked == true)
            {
                ...
                {
                    ColumnView view = GridView1;
                    view.ActiveFilter.Add(view.Columns["NgayThemHS"],
                      new ColumnFilterInfo("[NgayThemHS] like '%" + Month + "/" + Day + "/" + Yeah + "%'", ""));
                    view.ActiveFilter.Add(view.Columns["TinhTrang"],
                      new ColumnFilterInfo("[TinhTrang] like '%" + TinhTrang.Text + "%'", ""));
                }
            }
        ...
        private void textBoxDiaChi_TextChanged(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["DiaChi"],
              new ColumnFilterInfo("[DiaChi] like '%" + textBoxDiaChi.Text + "%'", ""));
        }
        string IDPhuong;
        private void btXuLyHoSo_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString(), out ID1);
                var date = DateTime.Now;
                QLHTDT.FormPhu.QLKienTruc.DangKyCapPhepXD frm = new QLHTDT.FormPhu.QLKienTruc.DangKyCapPhepXD(date.Day.ToString(), date.Month.ToString(), date.Year.ToString());
                frm.Show();
            }
            Cursor = Cursors.Default;
        }
        ...
        private void tbSoThua_TextChanged(object sender, EventArgs e)
        {
            ColumnView view = GridView1;
        ...
        private void tbChuDauTu_TextChanged(object sender, EventArgs e)
        {
            ColumnView view = GridView1;
        ...
        private void tbSoTo_TextChanged(object sender, EventArgs e)
        {
            ColumnView view = GridView1;
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString(), out ID1);
                var date = DateTime.Now;
                QLHTDT.FormPhu.QLKienTruc.ChinhSuaHoSoXD frm = new QLHTDT.FormPhu.QLKienTruc.ChinhSuaHoSoXD();
                frm.Show();
            }
            Cursor = Cursors.Default;
        }
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Hồ sơ cấp phép xây dựng được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString(), out ID1);
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_HoSoXayDung_BY_ID] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Hồ sơ cấp phép xây dựng thành công", "Thông báo");
                    ...
                    KienTruc.XoaDoiTuong("Hồ sơ cấp phép xây dựng", ID1);
                    KienTruc.dataAdapterNK.Update(KienTruc.TBNK);
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Hồ sơ cấp phép xây dựng cần xóa", "Thông báo");
                }
            }
        }
    }
