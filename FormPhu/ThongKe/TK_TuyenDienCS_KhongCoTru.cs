...
{
    public partial class TK_TuyenDienCS_KhongCoTru : Form
    {
        ...
        string ThongKe;
        string Phuong = "";
        public TK_TuyenDienCS_KhongCoTru()
        {
            InitializeComponent();
        ...
        {
            string sql = "[PRC_TK_TuyenDienCS_KhongCoTruCS] " + MaHuyen + ", " + MaXa + "";
        ...
        private void TK_TuyenDienCS_KhongCoTru_Load(object sender, EventArgs e)
        {
        ...
        private void GridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Tuyến điện chiếu sáng")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Tuyến điện chiếu sáng này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Tuyến điện chiếu sáng \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý chiếu sáng\\Tuyến điện chiếu sáng.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Tuyến điện chiếu sáng")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Tuyến điện chiếu sáng", "Thông báo"); }
                                else
                                {
