...
{
    public partial class TK_DAQH_TienDo : Form
    {
        int AddQuan = 0;
        DataTable tb;
        DataTable tbcheck;
        ...
        public TK_DAQH_TienDo()
        {
            InitializeComponent();
        }
        void showgridControl1(string MaHuyen)
        {
            string sql = "[PRC_STATISTICAL_DOANQH] " + MaHuyen + ",null";
        ...
        private void TK_DAQH_TienDo_Load(object sender, EventArgs e)
        {
            ...
            string sql = "[PRC_TK_DAQH_ChiTiet] null";
            tb = new DataTable(); SqlConnection connection = new SqlConnection(QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            dataAdapter1 = new SqlDataAdapter(new SqlCommand(sql, connection));
            cmbl = new SqlCommandBuilder(dataAdapter1);
            dataAdapter1.Fill(tb);
