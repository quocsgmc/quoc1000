...
{
    /// <summary>
    /// Summary description for TruyVanKhongGian.
    /// </summary>
    [Guid("c943ff5c-769a-4ab2-afac-6b7f1e2a5db8")]
    [ClassInterface(ClassInterfaceType.None)]
    [ProgId("QLHTDT.FormPhu.TruyVanKG.TruyVanKhongGian")]
    public sealed class TruyVanKhongGian : BaseTool
    {
        ...
        {
            frm = new ChonLopTruyVanKGcs();
            frm.Show();
            tenlop = ChonLopTruyVanKGcs.TenLop;
        ...
        public override void OnMouseDown(int Button, int Shift, int X, int Y)
        {
            ...
            IPolygon pPoly = (IPolygon)pRubberBand.TrackNew(pActiveView.ScreenDisplay, null);
            /*
            ...
            pLayer = Global.getLayerbyName(Global.pActiveView.FocusMap, ChonLopTruyVanKGcs.TenLop);
            if (pLayer == null)
            ...
            else if (pLayer.Name == "Đường giao thông chính")
            {
                for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                {
                    ...
                    {
                        ICompositeLayer igroup = QLHTDT.FormChinh.KienTruc.axMapControl1.Map.Layer[i] as ICompositeLayer;
            ...
            else if (pLayer.Name == "Ranh giới khu quy hoạch")
            {
                for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                {
                    ...
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Ranh giới khu quy hoạch")
                    {
                        ICompositeLayer igroup = QLHTDT.FormChinh.KienTruc.axMapControl1.Map.Layer[i] as ICompositeLayer;
