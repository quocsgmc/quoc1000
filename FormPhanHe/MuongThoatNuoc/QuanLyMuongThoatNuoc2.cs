...
{
    public partial class QuanLyMuongThoatNuoc2 : Form
    {
        ...
        string sql = "[PRC_QUERYTABLEMuongThoatNuoc]";
        public static int ID1 ;
        public static int LoadLaiForm = 0;
        public static string MaHuyen = "null";
        string sql1 = "[PRC_QUERYMuongThoatNuoc_BY_ID] " + ID1 + "";//
        string sqlQuan = "[PRC_Query_TenHuyen_By_MAHuyen] null";//
        public QuanLyMuongThoatNuoc2()
        {
            InitializeComponent();
        ...
        private void QuanLyMuongThoatNuoc2_Load(object sender, EventArgs e)
        {
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenDuong"],
               new ColumnFilterInfo("[TenDuong] like '%" + txtTenMuong.Text + "%'", ""));
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.MuongThoatNuoc.ChinhSuaMuongThoatNuoc frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.ChinhSuaMuongThoatNuoc(ID1);
                frm.TopMost = true;
                frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Mương thoát nước kiệt hẻm")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Mương thoát nước kiệt hẻm này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Mương thoát nước kiệt hẻm\n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý thoát nước\\Mương thoát nước kiệt hẻm.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Mương thoát nước kiệt hẻm")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Mương thoát nước kiệt hẻm", "Thông báo"); }
                                else
                                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                string path = Properties.Settings.Default.PathData + "\\Quản lý thoát nước\\Mương thoát nước kiệt hẻm" + "\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString();
            ...
            {
                MessageBox.Show("Chưa chọn Mương thoát nước kiệt hẻm cần đính kèm file. Vui lòng thử lại", "Thông báo");
            }
        }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.MuongThoatNuoc.ThemMoiMuongThoatNuoc frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.ThemMoiMuongThoatNuoc();
            frm.TopMost = true;
            frm.ShowDialog();
            if (LoadLaiForm == 1)
            {
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                try
                {
                    string Caption = GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "TENMUONG").ToString();
                    cardView1.CardCaptionFormat = Caption;
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                    sql1 = "[PRC_QUERYMuongThoatNuoc_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Mương thoát nước được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_MuongThoatNuoc] " + ID1 + "";
                    ...
                    KienTruc.XoaDoiTuong("Mương thoát nước kiệt hẻm", ID1);
                    KienTruc.dataAdapterNK.Update(KienTruc.TBNK);

                    MessageBox.Show("Xóa Mương thoát nước thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Mương thoát nước cần xóa", "Thông báo");
                }
            }
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["TENMUONG"],
              new ColumnFilterInfo("[TENMUONG] like '%" + txtTenMuong.Text + "%'", ""));
        }
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                QLHTDT.FormPhanHe.MuongThoatNuoc.QuanLyToaDoTuyenMuong frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.QuanLyToaDoTuyenMuong();
                frm.Show();
            }
            Cursor = Cursors.Default;
        }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.MuongThoatNuoc.MuongThoatNuocKHShapefile frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.MuongThoatNuocKHShapefile();
            frm.TopMost = true;
            frm.Show();
            Cursor = Cursors.Default;
        }
