...
{
    public partial class ChinhSuaHoGa_DC : Form
    {
        ...
        int IDChinhSua = QLHTDT.FormPhanHe.MuongThoatNuoc.QuanLyHoGa_DC.QuanLyHoGa_DC.ID1;
        public ChinhSuaHoGa_DC()
        {
            InitializeComponent();
            if (QLHTDT.FormChinh.KienTruc.FeatureWorkspace != null)
            { featureWorkspaceSDE = QLHTDT.FormChinh.KienTruc.FeatureWorkspace; }
        }
        private void ChinhSuaHoGa_DC_Load(object sender, EventArgs e)
        {
            comboHeToaDo.Text = "VN2000";

            SqlDataAdapter adp2 = new SqlDataAdapter("Select TENMUONG,OBJECTID from MUONGDC", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            ...
            SqlDataAdapter adp1 = new SqlDataAdapter("Select TENLOAI,IDLOAI from HOGADC_TYPE", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            DataSet ds1 = new DataSet();
            adp1.Fill(ds1);
            comboBox1.DataSource = ds1.Tables[0];
            comboBox1.DisplayMember = "TENLOAI";
            comboBox1.ValueMember = "IDLOAI";

            if (IDChinhSua != 0)
            {
                SqlDataAdapter adp3 = new SqlDataAdapter("[PRC_QUERY_TABLE_HoGa_DC_BY_ID] " + IDChinhSua + "", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            ...
            if (check != false)
            {
                try
                {
                    if (comboHeToaDo.Text == "VN2000")
                    {
                        ...
                        KienTruc.ChinhSuathuoctinhToolQuanLy("Hố ga chính", objectid);
                        KienTruc.dataAdapterNK.Update(KienTruc.TBNK);

                        MessageBox.Show("Chỉnh sửa Hố ga chính thành công", "Thông báo");
                    ...
                    else
                    {
                        ...
                        SqlConnection conn = new SqlConnection(QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
                        conn.Open();
                        string sql1 = "[PRC_UPDATE_HoGaDC_WGS] "
                            + " '" + objectid
                            + "', " + TenMuong
                            + ", " + LoaiHoGa
                            + ", " + DKinh
                ...
                catch
                {
                    ...
                    if (feature != null)
                    { feature.Delete(); }
                    MessageBox.Show("Chỉnh sửa Hố ga chính thất bại. Vui lòng kiểm tra dữ liệu", "Thông báo");
