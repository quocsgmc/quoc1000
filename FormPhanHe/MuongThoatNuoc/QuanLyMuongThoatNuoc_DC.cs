...
{
    public partial class QuanLyMuongThoatNuoc_DC : Form
    {
        ...
        string sql = "[PRC_QUERYTABLEMuongThoatNuocDC]";
        public static int ID1 ;
        public static int LoadLaiForm = 0;
        public static string MaHuyen = "null";
        string sql1 = "[PRC_QUERYMuongThoatNuocDC_BY_ID] " + ID1 + "";//
        string sqlQuan = "[PRC_Query_TenHuyen_By_MAHuyen] null";//
        public QuanLyMuongThoatNuoc_DC()
        {
            InitializeComponent();
        ...
        private void QuanLyMuongThoatNuoc_DC_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.MuongThoatNuoc.ChinhSuaMuongThoatNuoc_DC frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.ChinhSuaMuongThoatNuoc_DC(ID1);
                frm.TopMost = true;
                frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Mương thoát nước chính")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Mương thoát nước chính này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Mương thoát nước chính\n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý thoát nước\\Mương thoát nước chính.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Mương thoát nước chính")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Mương thoát nước chính", "Thông báo"); }
                                else
                                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                string path = Properties.Settings.Default.PathData + "\\Mương thoát nước\\Mương thoát nước chính" + "\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString();
            ...
            {
                MessageBox.Show("Chưa chọn Mương thoát nước chính cần đính kèm file. Vui lòng thử lại", "Thông báo");
            }
        }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.MuongThoatNuoc.ThemMoiMuongThoatNuoc_DC frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.ThemMoiMuongThoatNuoc_DC();
            frm.TopMost = true;
            frm.ShowDialog();
            if (LoadLaiForm == 1)
            {
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                    sql1 = "[PRC_QUERYMuongThoatNuocDC_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa Mương thoát nước được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_MuongThoatNuocDC] " + ID1 + "";
                    ...
                    KienTruc.XoaDoiTuong("Mương thoát nước chính", ID1);
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                QLHTDT.FormPhanHe.MuongThoatNuoc.QuanLyToaDoTuyenMuong_DC frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.QuanLyToaDoTuyenMuong_DC();
                frm.Show();
            }
            Cursor = Cursors.Default;
        }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.MuongThoatNuoc.MuongThoatNuocChinhShapefile frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.MuongThoatNuocChinhShapefile();
            frm.TopMost = true;
            frm.Show();
            Cursor = Cursors.Default;
        }
