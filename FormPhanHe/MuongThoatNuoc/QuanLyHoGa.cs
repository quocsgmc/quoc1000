...
namespace QLHTDT.FormPhanHe.MuongThoatNuoc.QuanLyHoGa
{
    public partial class QuanLyHoGa : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_HoGa]";
        public static int ID1 ;
        public static int LoadLaiForm = 0;
        public static string MaHuyen = "null";
        string sql1 = "[PRC_QUERY_TABLE_HoGa_BY_ID] " + ID1 + "";//
        ...
        private void QuanLyHoGa_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            {
                QLHTDT.FormPhanHe.MuongThoatNuoc.ChinhSuaHoGa frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.ChinhSuaHoGa();
                frm.TopMost = true;
                frm.ShowDialog();
                if (LoadLaiForm == 1)
                {
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Hố ga kiệt hẻm")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Hố ga kiệt hẻm này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Hố ga kiệt hẻm \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý thoát nước\\Hố ga kiệt hẻm.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Hố ga kiệt hẻm")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Hố ga kiệt hẻm", "Thông báo"); }
                                else
                                {
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.MuongThoatNuoc.ThemMoiHoGa frm = new QLHTDT.FormPhanHe.MuongThoatNuoc.ThemMoiHoGa();
            frm.TopMost = true;
            frm.ShowDialog();
            if (LoadLaiForm == 1)
            {
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                try
                {
                    ...
                    sql1 = "[PRC_QUERY_TABLE_HoGa_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                    //Lấy URL từ SQL
                    string server = QLHTDT.FormPhanHe.URLWeb.URL;
                    DataSet ds3 = new DataSet();
                    string GetSQL = "Select top(1) URL,NAME from HOGA_ASSET where IDHOGA = " + ID1 + " and (URL LIKE '%.jpg' or URL LIKE '%.png') ";
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa Mương thoát nước được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_HoGa] " + ID1 + "";
                    ...
                    KienTruc.XoaDoiTuong("Hố ga kiệt hẻm", ID1);
