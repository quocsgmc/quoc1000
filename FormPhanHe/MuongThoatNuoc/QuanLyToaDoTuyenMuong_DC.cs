...
{
    public partial class QuanLyToaDoTuyenMuong_DC : Form
    {
        ...
        int IDTuyenCapNgam = QLHTDT.FormPhanHe.MuongThoatNuoc.QuanLyMuongThoatNuoc_DC.QuanLyMuongThoatNuoc_DC.ID1;
        int IDCHUDAUTU;
        DataTable tt; DataRow dr;
        //string sql = "select a.OBJECTID,a.DiaChi,b.TENLOAITRAM,c.TENCHUDATU,d.TenPhuong,e.TENHUYEN from TRAMMuongThoatNuoc a,LOAITRAMMuongThoatNuoc b,CHUDAUTUMuongThoatNuoc c,PhuongXa d,QuanHuyen e where a.IDChuDauTu = c.IDCHUDAUTU and a.IDLoaiTram = b.IDLOAITRAM and a.MaXa = d.MaPhuong and e.MAHUYEN = a.MaHuyen";
        
        public QuanLyToaDoTuyenMuong_DC()
        {
            InitializeComponent();
        ...
        private void QuanLyToaDoTuyenMuong_DC_Load(object sender, EventArgs e)
        {
        ...
        {
            string sql = "[PRC_ToaDoTuyenMuongDC_GET_BY_ID] " + IDTuyenCapNgam + "";
            tb = new DataTable();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn lưu các dữ liệu thay đổi không?", "Lưu dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    string shape = "[PRC_UPDATE_SHAPE_MuongThoatNuocDC] " + IDTuyenCapNgam + ", 'LINESTRING (";
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            ...
            ThemMoiToaDo1DiemTuyenMuong_DC frm = new ThemMoiToaDo1DiemTuyenMuong_DC();
            frm.ShowDialog();
            if (frm.Visible == false)
            {
                DataRow barackRow = getRow(tb, ThemMoiToaDo1DiemTuyenMuong_DC.X, ThemMoiToaDo1DiemTuyenMuong_DC.Y);
                tb.Rows.InsertAt(barackRow, ThemMoiToaDo1DiemTuyenMuong_DC.STT);
            }
            converttable();
            GridView1.RefreshData();
        }
