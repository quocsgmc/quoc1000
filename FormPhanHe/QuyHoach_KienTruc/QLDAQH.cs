...
namespace QLHTDT.FormPhanHe.QuyHoach_KienTruc.QLDAQH
{
    public partial class QLDAQH : Form
    {
        ...
        string sql = "[PRC_QUERYTABLERGDAQH1]";
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["Tendoan"],
               new ColumnFilterInfo("[Tendoan] like '%" + textBox1.Text + "%'", ""));
        }
            ...
            try
            {
                for (int i1 = 0; i1 < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i1++)
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Ranh giới quy hoạch chi tiết")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Ranh giới quy hoạch quy hoạch 1/500 này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Ranh giới quy hoạch 1/500\n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Ranh giới quy hoạch chi tiết")
                            {
                                ...
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID_1").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Ranh giới quy hoạch 1/500", "Thông báo"); }
                                else
                                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                int IDTram;
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID_1").ToString(), out IDTram);
                string SQLLoad = "select Tenfiledinhkem,OBJECTID from [Doanquyhoach_Fliedinhkem] where Madoan = " + IDTram + "";
                string Dowload = "connect_file/duanqh/";
                string API = "api/duanqh/update-file";
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Đồ án quy hoạch 1/500 được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(cardView1.GetRowCellValue(cardView1.FocusedRowHandle, "OBJECTID_1").ToString(), out ID1);
                try
                {
                    string str = QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH);
                    SqlConnection conn = new SqlConnection(str);
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_RGDAQH_BY_ID] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Đồ án quy hoạch thành công", "Thông báo");
                    ...
                    KienTruc.XoaDoiTuong("Đồ án quy hoạch 1/500", ID1);
                    KienTruc.dataAdapterNK.Update(KienTruc.TBNK);
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Đồ án quy hoạch 1/500 cần xóa", "Thông báo");
                }
            }
        }
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID_1").ToString(), out IDTram);
                int IDPhuong;
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "Maphuong").ToString(), out IDPhuong);
                string SQLLoad = "select Name,OBJECTID from [BanDoGiayDAQHChiTiet] where Madoan = '" + IDTram + "' or Madoan = '" + IDTram + "A'";
                string Dowload = "connect_file/BanDoGiay/"+IDPhuong+"/";
                string API = "api/duanqh/update-file-ban-do-giay";
                if (IDTram != null)
                {
        ...
        {
            int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "Madoan").ToString(), out ID1);
            QLHTDT.FormPhu.ThongKe.TK_DAQH_ChiTiet frm = new QLHTDT.FormPhu.ThongKe.TK_DAQH_ChiTiet(ID1);
            frm.Show();
        }
    }
}
