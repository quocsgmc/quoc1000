...
namespace QLHTDT.FormPhanHe.QuyHoach_KienTruc.QLDAQH_PhanKhu
{
    public partial class QLDAQH_PhanKhu : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_QuyHoachPK]";
        public static int ID1 ;
        public static int LoadLaiForm = 0;
        string sql1 = "[PRC_QUERY_TABLE_QuyHoachPK_BY_ID] " + ID1 + "";
        string sqlQuan = "[PRC_Query_TenHuyen_By_MAHuyen] null";
        public static string MaHuyen = "null";
        public QLDAQH_PhanKhu()
        {
            InitializeComponent();
        ...
        private void QLDAQH_PhanKhu_Load(object sender, EventArgs e)
        {
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenPhanKhu"],
               new ColumnFilterInfo("[TenPhanKhu] like '%" + textBox2.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["NamQH"],
                new ColumnFilterInfo("[NamQH] like '%" + textBox1.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["QuyetDinhPhapLy"],
                new ColumnFilterInfo("[QuyetDinhPhapLy] like '%" + textBox3.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.QuyHoach_KienTruc.ChinhSuaDAQH_PhanKhu frm = new QLHTDT.FormPhanHe.QuyHoach_KienTruc.ChinhSuaDAQH_PhanKhu(ID1);
                frm.TopMost = true;
                frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Ranh giới quy hoạch phân khu")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Ranh giới quy hoạch phân khu này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Ranh giới quy hoạch phân khu \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quy hoạch phân khu 1.5000\\Ranh giới quy hoạch phân khu.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Ranh giới quy hoạch phân khu")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Ranh giới quy hoạch phân khu", "Thông báo"); }
                                else
                                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string SQLLoad = "select Tenfiledinhkem,OBJECTID from [QuyHoachPhanKhu_FileDinhKem] where MaPhanKhu = " + IDTram + "";
                string Dowload = "connect_file/quyhoachphankhu/";
                string API = "api/qh-phan-khu/update-file";
                if (IDTram != null)
                {
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                try
                {
                    string Caption = GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "TenPhanKhu").ToString();
                    cardView1.CardCaptionFormat = Caption;
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_TABLE_QuyHoachPK_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa Đồ án quy hoạch được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_QuyHoachPK_BY_ID] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Đồ án quy hoạch phân khu thành công", "Thông báo");
                    ...
                    KienTruc.XoaDoiTuong("Đồ án quy hoạch phân khu 1/5000", ID1);
                    KienTruc.dataAdapterNK.Update(KienTruc.TBNK);
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Đồ án quy hoạch cần xóa", "Thông báo");
                }
            }
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            {
                int IDTram;
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "MaPhanKhu").ToString(), out IDTram);
                string SQLLoad = "select Name,OBJECTID from [BanDoGiayQHPhanKhu] where MaPhanKhu = '" + IDTram + "' or MaPhanKhu = '" + IDTram + "A'";
                string Dowload = "connect_file/BanDoGiayQHPhanKhu/20314/";
                string API = "api/qh-phan-khu/update-file-ban-do-giay";
                if (IDTram != null)
                {
