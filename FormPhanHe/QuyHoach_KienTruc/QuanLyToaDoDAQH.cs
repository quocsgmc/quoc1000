...
using QLHTDT.FormPhanHe.ChieuSang;
using System;
...
{
    public partial class QuanLyToaDoDAQH : Form
    {
        ...
        int IDTuyenCapNgam = QLHTDT.FormPhanHe.ChieuSang.QuanLyTuyenDienChieuSang.ID1;
        int IDCHUDAUTU;
        DataTable tt; DataRow dr;
        //string sql = "select a.OBJECTID,a.DiaChi,b.TENLOAITRAM,c.TENCHUDATU,d.TenPhuong,e.TENHUYEN from TRAMChieuSang a,LOAITRAMChieuSang b,CHUDAUTUChieuSang c,PhuongXa d,QuanHuyen e where a.IDChuDauTu = c.IDCHUDAUTU and a.IDLoaiTram = b.IDLOAITRAM and a.MaXa = d.MaPhuong and e.MAHUYEN = a.MaHuyen";
        
        public QuanLyToaDoDAQH()
        {
            InitializeComponent();
        ...
        private void QuanLyToaDoDAQH_Load(object sender, EventArgs e)
        {
        ...
        {
            string sql = "[PRC_ToaDoDAQH_GET_BY_ID] " + IDTuyenCapNgam + "";
            tb = new DataTable();
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                string sql = "[PRC_QUERYTuyenDienCS_BY_ID] " + IDTuyenCapNgam + "";
                GridView1.MoveNext();
                GridView1.MovePrev();
                SqlConnection connection = new SqlConnection(QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
                dataAdapter1 = new SqlDataAdapter(new SqlCommand(sql, connection));
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn lưu các dữ liệu thay đổi không?", "Lưu dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    string shape = "[PRC_UPDATE_SHAPE_DAQH] " + IDTuyenCapNgam + ", 'POLYGON (";
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            ...
            ThemMoiToaDo1DiemDAQH frm = new ThemMoiToaDo1DiemDAQH();
            frm.ShowDialog();
            if (frm.Visible == false)
            {
                DataRow barackRow = getRow(tb, ThemMoiToaDo1DiemDAQH.X, ThemMoiToaDo1DiemDAQH.Y);
                tb.Rows.InsertAt(barackRow, ThemMoiToaDo1DiemDAQH.STT);
            }
            converttable();
            GridView1.RefreshData();
        }
        ...
        {
            double X = double.Parse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "X").ToString());//Lấy tọa độ X
            double Y = double.Parse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "Y").ToString());//Lấy tọa độ Y
