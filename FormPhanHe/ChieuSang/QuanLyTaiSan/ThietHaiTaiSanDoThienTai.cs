...
namespace QLHTDT.FormPhanHe.ChieuSang.QuanLyTaiSan.ThietHaiDoThienTai
{
    public partial class ThietHaiTaiSanDoThienTai : Form
    {
        ...
        string sql = "SELECT [OBJECTID],[STT],[ChuDauTu],[DiaChi],[PhuongXa],[QuanHuyen],[KinhDo] ,[ViDo] ,[LoaiTru] ,[DoCao] ,[Ngay] ,[SoGP] ,[NgayCapGP] ,[SoCN] ,[NgayVaoSoCN],[DungChung] ,[Khac] ,[ThongTinKh]  ,[XVN2000],[YVN2000] FROM  TRAMChieuSang";

        public ThietHaiTaiSanDoThienTai()
        {
            InitializeComponent();
            dmap = QLHTDT.FormChinh.KienTruc.axMapControl1.Map;
        }

        private void ThietHaiTaiSanDoThienTai_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (ChinhSua == true)
                {
                    ...
                    //ChieuSang.dataAdapterNK = new SqlDataAdapter(new SqlCommand("Select * From NhatKy", connectionNK));
                    //cmbl = new SqlCommandBuilder(ChieuSang.dataAdapterNK);
                    //ChieuSang.dataAdapterNK.Fill(ChieuSang.TBNK);
                    ////ChieuSang.ChinhSuathuoctinhToolQuanLy("Đồ án Quy hoạch " + comboBox1.Text);
                    //ChieuSang.dataAdapterNK.Update(ChieuSang.TBNK);
                }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Trạm ChieuSang")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian trạm ChieuSang quy hoạch này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Trạm ChieuSang \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Trạm ChieuSang.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Trạm ChieuSang")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Trạm ChieuSang", "Thông báo"); }
                                else
                                {
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.ChieuSang.LapKeHoachBaoDuong.LapKeHoachBaoDuong frm = new QLHTDT.FormPhanHe.ChieuSang.LapKeHoachBaoDuong.LapKeHoachBaoDuong();
            frm.Show();
            Cursor = Cursors.Default;
        }
    }
