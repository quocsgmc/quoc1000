...
{
    public partial class QuanLyTuyenDienChieuSang : Form
    {
        ...
        string sql = "[PRC_QUERYTABLETuyenDienCS]";
        ...
        public QuanLyTuyenDienChieuSang()
        {
            InitializeComponent();
        ...
        private void QuanLyTuyenDienChieuSang_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.ChieuSang.ChinhSuaTuyenDienCS frm = new QLHTDT.FormPhanHe.ChieuSang.ChinhSuaTuyenDienCS(ID1);
                frm.TopMost = true;
                frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Tuyến điện chiếu sáng kiệt hẻm")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Tuyến điện chiếu sáng kiệt hẻm này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Tuyến điện chiếu sáng kiệt hẻm\n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý chiếu sáng\\Tuyến điện chiếu sáng kiệt hẻm.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Tuyến điện chiếu sáng kiệt hẻm")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Tuyến điện chiếu sáng kiệt hẻm", "Thông báo"); }
                                else
                                {
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.ChieuSang.ThemMoiTuyenDienCS frm = new QLHTDT.FormPhanHe.ChieuSang.ThemMoiTuyenDienCS();
            frm.TopMost = true;
            frm.ShowDialog(this);
            if (LoadLaiForm == 1)
            {
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.ChieuSang.TuyenDienKHShapefile frm = new QLHTDT.FormPhanHe.ChieuSang.TuyenDienKHShapefile();
            frm.Show();
            Cursor = Cursors.Default;
        }
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                    sql1 = "[PRC_QUERYTuyenDienCS_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa Tuyến điện chiếu sáng được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_TUYENDIENCS_BY_ID] " + ID1 + "";
                    ...
                    KienTruc.XoaDoiTuong("Tuyến điện chiếu sáng kiệt hẻm", ID1);
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["TENTUYEN"],
              new ColumnFilterInfo("[TENTUYEN] like '%" + txtTenTru.Text + "%'", ""));
        }
        ...
        private void button7_Click_1(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                QLHTDT.FormPhanHe.ChieuSang.QuanLyToaDoTuyenDienCS frm = new QLHTDT.FormPhanHe.ChieuSang.QuanLyToaDoTuyenDienCS();
                frm.Show();
            }
            Cursor = Cursors.Default;
        }
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                this.ClientSize = new System.Drawing.Size(765, 795);
                MoRong = 0;
                this.button8.Image = global::QLHTDT.Properties.Resources.MuiTen;
            }
        }
    }
}
