...
{
    public partial class QuanLyTruDienCS_DC : Form
    {
        ...
        string sql = "[PRC_QUERYTABLETruDienCSDC]";
        public static int ID1 ;
        public static int LoadLaiForm;
        public static string MaHuyen = "null";
        string sql1 = "[PRC_QUERYTruDienCSDC_BY_ID] " + ID1 + "";//
        string sqlQuan = "[PRC_Query_TenHuyen_By_MAHuyen] null";//
        public QuanLyTruDienCS_DC()
        {
            InitializeComponent();
        ...
        private void QuanLyTruDienCS_DC_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter adp1 = new SqlDataAdapter("Select * from TUYENDIENCSDC", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            DataSet ds1 = new DataSet();
            adp1.Fill(ds1);
            cboTuyenDien.DataSource = ds1.Tables[0];
            cboTuyenDien.DisplayMember = "TENTUYEN";
            cboTuyenDien.ValueMember = "OBJECTID";
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.ChieuSang.ChinhSuaTruDienCS_DC frm = new QLHTDT.FormPhanHe.ChieuSang.ChinhSuaTruDienCS_DC(ID1);
                frm.TopMost = true;
                frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Trụ điện chiếu sáng chính")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Trụ điện chiếu sáng chính này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Trụ điện chiếu sáng chính\n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý chiếu sáng\\Trụ điện chiếu sáng chính.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Trụ điện chiếu sáng")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Trụ điện chiếu sáng chính", "Thông báo"); }
                                else
                                {
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.ChieuSang.ThemMoiTruDienCS_DC frm = new QLHTDT.FormPhanHe.ChieuSang.ThemMoiTruDienCS_DC();
            frm.TopMost = true;
            frm.ShowDialog(this);
            if (LoadLaiForm == 1)
            {
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.ChieuSang.ThemMoiExcelTruDienCS_DC frm = new QLHTDT.FormPhanHe.ChieuSang.ThemMoiExcelTruDienCS_DC();
            frm.Show();
            Cursor = Cursors.Default;
        }
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                    sql1 = "[PRC_QUERYTruDienCSDC_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                    //Lấy URL từ SQL
                    string server = QLHTDT.FormPhanHe.URLWeb.URL;
                    DataSet ds3 = new DataSet();
                    string GetSQL = "Select top(1) URL from TRUDIENCSDC_ASSET where IDTRUDIENCSDC = " + ID1 + " and (URL LIKE '%.jpg' or URL LIKE '%.png') ";
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa Trụ điện chiếu sáng được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_TRUDIENCSDC_BY_ID] " + ID1 + "";
        ...
        private void cboTuyenDien_SelectedIndexChanged(object sender, EventArgs e)
        {
            AddQuan = 1;
            GridView1.ClearColumnsFilter();
            GridView1.RefreshData();
            ...
            if (cboTuyenDien.SelectedValue.ToString() == "System.Data.DataRowView")
            {
                AddQuan = 0;
                cboTuyenDien.Text = "";
            }
            if (AddQuan == 1)
            {
                view.ActiveFilter.Add(view.Columns["IDTUYENDIENCSDC"],
              new ColumnFilterInfo("[IDTUYENDIENCSDC] like '%" + cboTuyenDien.SelectedValue.ToString() + "%'", ""));
            }
