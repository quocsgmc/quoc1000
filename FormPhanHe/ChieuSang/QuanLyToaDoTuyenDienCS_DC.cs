...
{
    public partial class QuanLyToaDoTuyenDienCS_DC : Form
    {
        ...
        int IDTuyenCapNgam = QLHTDT.FormPhanHe.ChieuSang.QuanLyTuyenDienChieuSang_DC.ID1;
        int IDCHUDAUTU;
        ...
        public QuanLyToaDoTuyenDienCS_DC()
        {
            InitializeComponent();
        ...
        private void QuanLyToaDoTuyenDienCS_DC_Load(object sender, EventArgs e)
        {
        ...
        {
            string sql = "[PRC_ToaDoTuyenDienCSDC_GET_BY_ID] " + IDTuyenCapNgam + "";
            tb = new DataTable();
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                string sql = "[PRC_QUERYTuyenDienCSDC_BY_ID] " + IDTuyenCapNgam + "";
                GridView1.MoveNext();
                GridView1.MovePrev();
                SqlConnection connection = new SqlConnection(QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
                dataAdapter1 = new SqlDataAdapter(new SqlCommand(sql, connection));
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn lưu các dữ liệu thay đổi không?", "Lưu dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    string shape = "[PRC_UPDATE_SHAPE_TUYENDIENCSDC] " + IDTuyenCapNgam + ", 'LINESTRING (";
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            ...
            ThemMoiToaDo1DiemTuyenDienCS_DC frm = new ThemMoiToaDo1DiemTuyenDienCS_DC();
            frm.ShowDialog();
            if (frm.Visible == false)
            {
                DataRow barackRow = getRow(tb, ThemMoiToaDo1DiemTuyenDienCS_DC.X, ThemMoiToaDo1DiemTuyenDienCS_DC.Y);
                tb.Rows.InsertAt(barackRow, ThemMoiToaDo1DiemTuyenDienCS_DC.STT);
            }
            converttable();
            GridView1.RefreshData();
        }
