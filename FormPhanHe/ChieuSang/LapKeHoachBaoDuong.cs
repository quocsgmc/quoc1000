...
namespace QLHTDT.FormPhanHe.ChieuSang.LapKeHoachBaoDuong
{
    public partial class LapKeHoachBaoDuong : Form
    {
        ...
        public LapKeHoachBaoDuong()
        {
            InitializeComponent();
            dmap = QLHTDT.FormChinh.KienTruc.axMapControl1.Map;
        }

        private void LapKeHoachBaoDuong_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i1 = 0; i1 < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i1++)
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Trạm ChieuSang")
                    {
                        ...
                        KTMoLop = KTMoLop + 1;
                        int ID;
                        int.TryParse(gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                        if (ID == 0)
                        {
                            Cursor = Cursors.WaitCursor;
