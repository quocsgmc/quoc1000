...
{
    public partial class ChinhSuaTruDienCS : Form
    {
        ...
        public ChinhSuaTruDienCS(int OBJECTID)
        {
            InitializeComponent();
        ...
        private void ChinhSuaTruDienCS_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter adp2 = new SqlDataAdapter("Select * from TuyenDienCS", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            DataSet ds2 = new DataSet();
            adp2.Fill(ds2);
            comboTuyenDay.DataSource = ds2.Tables[0];
            comboTuyenDay.DisplayMember = "TENTUYEN";
            comboTuyenDay.ValueMember = "OBJECTID";

            if (IDChinhSua != 0)
            {
                SqlDataAdapter adp3 = new SqlDataAdapter("[PRC_QUERYTruDienCS_BY_ID] " + IDChinhSua + "", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
        ...
        {
            if (MessageBox.Show("Bạn muốn Chỉnh sửa Trụ điện CS kiệt hẻm không?", "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                if (check != false)
                {
                    try
                    {
                        if (comboHeToaDo.Text == "VN2000")
                        {
                            ...
                            KienTruc.ChinhSuathuoctinhToolQuanLy("Trụ điện chiếu sáng kiệt hẻm", objectid);
