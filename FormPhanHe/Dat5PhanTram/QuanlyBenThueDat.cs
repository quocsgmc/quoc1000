...
{
    public partial class QuanlyBenThueDat : Form
    {
        ...
        public static int IDThueDat = 0;
        public QuanlyBenThueDat()
        {
            InitializeComponent();
            dmap = QLHTDT.FormChinh.KienTruc.axMapControl1.Map;
        }
        int ID1 = QLHTDT.FormPhanHe.Dat5PhanTram.QuanlyDat5PhanTram.QuanlyDat5PhanTram.ID1;
        private void QuanlyBenThueDat_Load(object sender, EventArgs e)
        {
            ...
            if (ID1 != 0)
            {
                string sql1 = "[PRC_QUERY_TABLE_BenThueDat] " + ID1 + "";
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(cardView1.GetRowCellValue(cardView1.FocusedRowHandle, "ID").ToString(), out IDThueDat);
                QLHTDT.FormPhanHe.Dat5PhanTram.ChinhSuaBenThueDat frm = new QLHTDT.FormPhanHe.Dat5PhanTram.ChinhSuaBenThueDat();
                frm.Show();
            }
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Bên thuê đất không " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(cardView1.GetRowCellValue(cardView1.FocusedRowHandle, "ID").ToString(), out ID1);
                try
                {
                    string str = QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH);
                    SqlConnection conn = new SqlConnection(str);
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_BenThueDat] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    MessageBox.Show("Xóa Bên thuê đất thành công", "Thông báo");
                    string sql1 = "[PRC_QUERY_TABLE_BenThueDat] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
                    MessageBox.Show("Không có dữ liệu cần xóa cần xóa", "Thông báo");
                }
            }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.Dat5PhanTram.ThemMoiBenThueDat frm = new QLHTDT.FormPhanHe.Dat5PhanTram.ThemMoiBenThueDat();
            frm.Show();
            this.Close();
            //if (LoadLaiForm == 1)
            //{
