...
{
    public partial class QuanLyCotCapTreo : Form
    {
        ...
        private void QuanLyCotCapTreo_Load(object sender, EventArgs e)
        {
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["IDTuyen"],
               new ColumnFilterInfo("[IDTuyen] like '%" + txtMaSoCot.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["LoaiCap"],
               new ColumnFilterInfo("[LoaiCap] like '%" + comboLoaiCot.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCotCapTreo").ToString(), out ID1);
                QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaCotCapTreo frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaCotCapTreo();
                frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Cột cáp treo")
                    {
                        IFeatureLayer ilayer1 = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1) as IFeatureLayer;
                        KTMoLop = KTMoLop + 1;
                        int ID;
                        int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCotCapTreo").ToString(), out ID);
                        if (ID == 0)
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Cột cáp treo này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Cột cáp treo \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Bưu chính - Viễn thông\\Tuyến cáp\\Cột cáp treo.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Cột cáp treo")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCotCapTreo").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Cột cáp treo", "Thông báo"); }
                                else
                                {
        ...
        private void comboQuan_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["OBJECTID"],
               new ColumnFilterInfo("[OBJECTID] like '%" + txtMaSoCot.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["DiaChi"],
               new ColumnFilterInfo("[DiaChi] like '%" + comboLoaiCot.Text + "%'", ""));
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["IDBECAP"],
               new ColumnFilterInfo("[IDBECAP] like '%" + txtMaSoCot.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["TenDonViSoHuu"],
               new ColumnFilterInfo("[TenDonViSoHuu] like '%" + comboLoaiCot.Text + "%'", ""));
        ...
        {
            txtMaSoCot.ResetText();
            comboLoaiCot.ResetText();
            comboQuan.ResetText();
            comboPhuong.ResetText();
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["MaSoCot"],
              new ColumnFilterInfo("[MaSoCot] like '%" + txtMaSoCot.Text + "%'", ""));
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string path = Properties.Settings.Default.PathData + "\\Bưu chính - Viễn thông\\Tuyến cáp\\Cột cáp treo" + "\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "MaSoCot").ToString();
            ...
            {
                MessageBox.Show("Chưa chọn cột cáp cần đính kèm file. Vui lòng thử lại","Thông báo");
            }
        }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ThemMoiCotCapTreo frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ThemMoiCotCapTreo();
            frm.ShowDialog();
            if (LoadLaiForm == 1)
            {
                showgridControl1();
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCotCapTreo").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_COTCAP_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                    return;
                }
                catch
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Cột cáp treo được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCotCapTreo").ToString(), out ID1);
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_COTCAP_BY_ID] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Cột cáp treo thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Cột cáp treo cần xóa", "Thông báo");
                }
            }
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["LoaiCot"],
               new ColumnFilterInfo("[LoaiCot] like '%" + comboLoaiCot.Text + "%'", ""));
        }
    }
}
