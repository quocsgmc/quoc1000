...
{
    public partial class QuanLyDoanhNghiep : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_DOANHNGHIEP]";
        public QuanLyDoanhNghiep()
        {
            InitializeComponent();
        ...
        private void QuanLyDoanhNghiep_Load(object sender, EventArgs e)
        {
        ...
        void showgridControl1()
        {
            ...
            {
                var val = ds.Tables[0].Rows[intCount]["TenVietTat"].ToString();
                if (!comboTenVietTat.Items.Contains(val))
                {
                    comboTenVietTat.Items.Add(val);
                }
        ...
        private void BtExcell_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa dữ liệu được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDDoanhNghiep").ToString(), out IDCHUDAUTU);
                    ...
                    string Delelequery = "[PRC_DELETE_DOANHNGHIEP] " + IDCHUDAUTU + "";
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["STT"],
               new ColumnFilterInfo("[STT] like '%" + txtTenDN.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (IDTram >= 0)
            {
                IDTram = int.Parse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDDoanhNghiep").ToString());
                QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaDoanhNghiep frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaDoanhNghiep();
                frm.Show();
                this.Close();
            }
            else
            {
                MessageBox.Show("Vui lòng chọn doanh nghiệp cần chỉnh sửa.", "Thông báo");
            }
        ...
        private void txtTenDN_TextChanged(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenDoanhNghiep"],
              new ColumnFilterInfo("[TenDoanhNghiep] like '%" + txtTenDN.Text + "%'", ""));
        }

        private void button2_Click(object sender, EventArgs e)
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ThemMoiDoanhNghiep frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ThemMoiDoanhNghiep();
            frm.Show();
            this.Close();
            Cursor = Cursors.Default;
        }

        private void comboTenVietTat_SelectedIndexChanged(object sender, EventArgs e)
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["TenDoanhNghiep"],
               new ColumnFilterInfo("[TenDoanhNghiep] like '%" + txtTenDN.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["TenVietTat"],
               new ColumnFilterInfo("[TenVietTat] like '%" + comboTenVietTat.Text + "%'", ""));
        }
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            ...
            {
                //IDTram = int.Parse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDDoanhNghiep").ToString());
                //txtTenDoanhNghiep.Text = GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "TenDoanhNghiep").ToString();
                //txtTenVietTat.Text = GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "TenVietTat").ToString();
