...
{
    public partial class QuanLyThietBiPhuTro : Form
    {
        ...
        string sql = "[PRC_QUERY_THIETBIPHUTRO_BY_LOAITB] null";
        public QuanLyThietBiPhuTro()
        {
            InitializeComponent();
        ...
        private void QuanLyThietBiPhuTro_Load(object sender, EventArgs e)
        {
        ...
        private void BtExcell_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa dữ liệu được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDThietBi").ToString(), out IDCHUDAUTU);
                    string str = QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH);
                    SqlConnection conn = new SqlConnection(str);
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_THIETBIPHUTRO] " + IDCHUDAUTU + "";
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            try
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDThietBi").ToString(), out IDThietBi);
                QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaThietBiPhuTro frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaThietBiPhuTro();
                frm.Show();
                this.Close();
            }
            catch
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["TenThietBi"],
              new ColumnFilterInfo("[TenThietBi] like '%" + txtTenTB.Text + "%'", ""));
        }
