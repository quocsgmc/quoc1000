...
{
    public partial class QuanLyCongTrinhHaTang : Form
    {
        ...
        private void QuanLyCongTrinhHaTang_Load(object sender, EventArgs e)
        {
        ...
        void showgridControl1()
        {
            ...
            {
                var val = ds.Tables[0].Rows[intCount]["MoTa"].ToString();
                if (!comboLoaiPhuTro.Items.Contains(val))
                {
                    comboLoaiPhuTro.Items.Add(val);
                }
            }
        }
        ...
        private void BtExcell_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa dữ liệu được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDLoaiCT").ToString(), out IDCHUDAUTU);
                    string str = QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH);
                    SqlConnection conn = new SqlConnection(str);
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_LOAICTVT] " + IDCHUDAUTU + "";
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["STT"],
               new ColumnFilterInfo("[STT] like '%" + txtTenTB.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            try
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDLoaiCT").ToString(), out IDThietBi);
                QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaCongTrinhHaTang frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaCongTrinhHaTang();
                frm.Show();
                this.Close();
            }
            catch
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["TenLoaiCT"],
              new ColumnFilterInfo("[TenLoaiCT] like '%" + txtTenTB.Text + "%'", ""));
        }

        private void button2_Click(object sender, EventArgs e)
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ThemMoiCongTrinhHaTang frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ThemMoiCongTrinhHaTang();
            frm.Show();
            this.Close();
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenLoaiCT"],
               new ColumnFilterInfo("[TenLoaiCT] like '%" + txtTenTB.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["MoTa"],
               new ColumnFilterInfo("[MoTa] like '%" + comboLoaiPhuTro.Text + "%'", ""));
        }
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            ...
            {
                //txtTenThietBi.Text = GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "TenLoaiCT").ToString();
