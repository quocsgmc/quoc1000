...
{
    public partial class QuanLyTuyenCapTreo : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_TuyenCapTreo]";
        public static int ID1 ;
        public static int LoadLaiForm = 0;

        public static string MaHuyen = "null";
        string sql1 = "[PRC_QUERY_TuyenCapTreo_BY_ID] " + ID1 + "";
        string sqlDiemDau = "[PRC_QUERY_TABLE_TuyenCapTreo]";
        string sqlDiemCuoi = "[PRC_QUERY_TABLE_TuyenCapTreo]";
        string sqlTenDoanhNghiep = "[PRC_QUERY_TABLE_DOANHNGHIEP]";
        public QuanLyTuyenCapTreo()
        {
            InitializeComponent();
        ...
        private void QuanLyTuyenCapTreo_Load(object sender, EventArgs e)
        {
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["IDTuyen"],
               new ColumnFilterInfo("[IDTuyen] like '%" + txtIDTuyen.Text + "%'", ""));
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaTuyenCapTreo frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ChinhSuaTuyenCapTreo();
                frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Tuyến cáp treo")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Tuyến cáp treo này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Tuyến cáp treo \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Bưu chính - Viễn thông\\Tuyến cáp\\Tuyến cáp treo.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Tuyến cáp treo")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDTuyen").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Tuyến cáp treo", "Thông báo"); }
                                else
                                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string path = Properties.Settings.Default.PathData + "\\Bưu chính - Viễn thông\\Tuyến cáp\\Tuyến cáp treo" + "\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDTuyen").ToString();
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ThemMoiTuyenCapTreo frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.ThemMoiTuyenCapTreo();
            frm.ShowDialog();
            if (LoadLaiForm == 1)
            {
                showgridControl1();
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDTuyen").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_TuyenCapTreo_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                    return;
                }
                catch
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Tuyến cáp treo được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDTuyen").ToString(), out ID1);
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_TUYENCAPTREO_BY_ID] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Tuyến cáp treo thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Tuyến cáp treo cần xóa", "Thông báo");
                }
            }
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDTuyen").ToString(), out ID1);
                QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.QuanLyToaDoTuyenCapTreo frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.QuanLyToaDoTuyenCapTreo();
                frm.Show();
            }
            Cursor = Cursors.Default;
        }
