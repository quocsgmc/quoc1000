...
{
    public partial class ChinhSuaCongTrinhHaTang : Form
    {
        DataTable tb;
        DataTable tbcheck;
        SqlDataAdapter dataAdapter1;
        SqlCommandBuilder cmbl;
        int IDThietBi = QLHTDT.FormPhanHe.BuuChinh_VienThong.TuyenCap.QuanLyCongTrinhHaTang.IDThietBi;
        public ChinhSuaCongTrinhHaTang()
        {
            InitializeComponent();
        }

        private void ChinhSuaCongTrinhHaTang_Load(object sender, EventArgs e)
        {
            DataSet ds3 = new DataSet();
            SqlDataAdapter adp3 = new SqlDataAdapter("[PRC_QUERY_TABLE_LOAICTVT_BY_ID] " + IDThietBi+"", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            adp3.Fill(ds3);
            txtTenThietBi.Text = ds3.Tables[0].Rows[0]["TenLoaiCT"].ToString();
            ...
            if (LoaiThietBi != 0)
            {
                if (MessageBox.Show("Bạn muốn chỉnh sửa thiết bị?", "Thêm mới thiết bị", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        conn.Open();
                        string update = "[PRC_UPDATE_LOAICTHTVT] " + IDThietBi+",N'" + txtTenThietBi.Text + "'," + LoaiThietBi + ",N'" + txtGhiChu.Text + "'";
