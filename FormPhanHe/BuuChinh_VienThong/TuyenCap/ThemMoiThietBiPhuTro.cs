...
{
    public partial class ThemMoiThietBiPhuTro : Form
    {
        DataTable tb;
        DataTable tbcheck;
        ...
        public ThemMoiThietBiPhuTro()
        {
            InitializeComponent();
        }

        private void ThemMoiThietBiPhuTro_Load(object sender, EventArgs e)
        {
            //showgridControl1();
        }
            ...
            if (LoaiThietBi != 0)
            {
                if (MessageBox.Show("Bạn muốn thêm mới một thiết bị?", "Thêm mới thiết bị", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        conn.Open();
                        string update = "[PRC_INSERT_THIETBIPHUTRO_BY_ID] N'" + txtTenThietBi.Text + "','" + LoaiThietBi + "',N'" + txtGhiChu.Text + "'";
