...
namespace QLHTDT.FormPhanHe.DiaChinh
{
    public partial class TraCuuDiaChinh : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_DiaChinh]";
        public static int ID1 ;
        public static int LoadLaiForm = 0;
        string sql1 = "[PRC_QUERY_TABLE_DiaChinh_BY_ID] " + ID1 + "";
        string sqlQuan = "[PRC_Query_TenHuyen_By_MAHuyen] null";
        public static string MaHuyen = "null";
        public TraCuuDiaChinh()
        {
            InitializeComponent();
        ...
        private void TraCuuDiaChinh_Load(object sender, EventArgs e)
        {
            GridView1.OptionsBehavior.Editable = false;
            vGridControl3.OptionsBehavior.Editable = false;
            //vGridControl1.OptionsBehavior.Editable = false;
            //vGridControl2.OptionsBehavior.Editable = false;
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["SoThua"],
               new ColumnFilterInfo("[SoThua] like '%" + textBox1.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["SoToBD"],
               new ColumnFilterInfo("[SoToBD] like '%" + textBox2.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            //    QLHTDT.FormPhanHe.QuyHoach_KienTruc.ChinhSuaDAQH frm = new QLHTDT.FormPhanHe.QuyHoach_KienTruc.ChinhSuaDAQH();
            //    frm.Show();
            //}
            ...
            else
            {
                ...
                //    KienTruc.ChinhSuathuoctinhToolQuanLy("Thông tin kiến trúc");
            ...
            try
            {
                ...
                for (int i1 = 0; i1 < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i1++)
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Địa chính")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian thông tin thửa đất này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian thông tin thửa đất \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Địa chính")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian thông tin thửa đất", "Thông báo"); }
                                else
                                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                string path = Properties.Settings.Default.PathData + "\\FileDinhKemKT\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "TenKhuVuc").ToString();
            ...
            {
                MessageBox.Show("Chưa chọn thông tin kiến trúc cần đính kèm file. Vui lòng thử lại", "Thông báo");
            }
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_TABLE_DiaChinh_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
