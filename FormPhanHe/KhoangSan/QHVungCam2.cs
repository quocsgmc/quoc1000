...
namespace QLHTDT.FormPhanHe.KhoangSan.QHVungCam2
{
    public partial class QHVungCam2 : Form
    {
        ...
        string sql1 = "PRC_QUERY_QuyHoachVungCam_BY_ID " + ID1 + "";
        public QHVungCam2()
        {
            InitializeComponent();
        ...
        private void QHVungCam2_Load(object sender, EventArgs e)
        {
            ...
            string sql4 = "SELECT [DoiTuong] FROM [QH_VUNGCAM]";
            SqlDataAdapter adp1 = new SqlDataAdapter(sql4, QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            adp1.FillSchema(ds2, SchemaType.Source);
            adp1.Fill(ds2);
            comboBox3.Items.Clear();
            comboBox3.Items.Add("");
            for (int intCount = 0; intCount < ds2.Tables[0].Rows.Count; intCount++)
            {
                var val = ds2.Tables[0].Rows[intCount]["DoiTuong"].ToString();
        ...
        void showgridControl1()
        {
            ...
            dataAdapter1 = new SqlDataAdapter(new SqlCommand("PRC_QUERY_QuyHoachVungCam", connection));
        ...
        {
            QLHTDT.FormPhanHe.KhoangSan.ChinhSuaQHVungCam frm = new QLHTDT.FormPhanHe.KhoangSan.ChinhSuaQHVungCam();
            frm.Show();
            ...
            //    cardView1.OptionsBehavior.Editable = false;
            //    tb = new DataTable();
            //    string sql = "SELECT [Id],[DoiTuong],[Quan],[DienTich],[TT],[KhuVucQH],[GhiChu],[Phuong],[OBJECTID] FROM [QH_VUNGCAM]";
            //    SqlConnection connection = new SqlConnection(QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Quy hoạch vùng cấm")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Quy hoạch vùng cấm này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Quy hoạch vùng cấm \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý tài nguyên khoáng sản\\Quy hoạch vùng cấm.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Quy hoạch vùng cấm")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Quy hoạch vùng cấm này", "Thông báo"); }
                                else
                                {
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlConnection connection = new SqlConnection(QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
                dataAdapter1 = new SqlDataAdapter(new SqlCommand("select SELECT [Id],[DoiTuong],[Quan],[DienTich],[TT],[KhuVucQH],[GhiChu],[Phuong],[OBJECTID] FROM [QH_VUNGCAM] FROM [QH_VUNGCAM]", connection));
                cmbl = new SqlCommandBuilder(dataAdapter1);
                ...
                dataAdapter2 = new SqlDataAdapter(new SqlCommand("select SELECT [Id],[DoiTuong],[Quan],[DienTich],[TT],[KhuVucQH],[GhiChu],[Phuong],[OBJECTID] FROM [QH_VUNGCAM] FROM [QH_VUNGCAM]", connection1));
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string path = Properties.Settings.Default.PathData + "\\Dữ liệu quy hoạch khoáng sản\\QuyHoachVungCam\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString();
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                try
                {
                    ...
                    sql1 = "PRC_QUERY_QuyHoachVungCam_BY_ID " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa quy hoạch được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_QuyHoachVungCam] " + ID1 + "";
        ...
        {
            QLHTDT.FormPhanHe.KhoangSan.ThemMoiQHVungCam frm = new QLHTDT.FormPhanHe.KhoangSan.ThemMoiQHVungCam();
            frm.Show();
        }
        ...
        private void comboBox3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["DoiTuong"],
              new ColumnFilterInfo("[DoiTuong] like '%" + comboBox3.Text + "%'", ""));
        }
    }
}
