...
{
    public partial class QuanLyViaHe : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_ViaHe]";
        public static int ID1 ;

        public static string MaHuyen = "null";
        string sql1 = "[PRC_QUERY_TABLE_ViaHe_BY_ID] " + ID1 + "";//
        ...
        private void QuanLyViaHe_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.GiaoThong.ChinhSua.ChinhSuaViaHe frm = new QLHTDT.FormPhanHe.GiaoThong.ChinhSua.ChinhSuaViaHe();
                frm.ShowDialog();
                showgridControl1();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Vỉa hè")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Vỉa hè này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Vỉa hè\n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý hạ tầng giao thông\\Vỉa hè.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Vỉa hè")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Vỉa hè", "Thông báo"); }
                                else
                                {
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.GiaoThong.ThemMoi.ThemMoiViaHe frm = new QLHTDT.FormPhanHe.GiaoThong.ThemMoi.ThemMoiViaHe();
            frm.Show();
            Cursor = Cursors.Default;
        }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.GiaoThong.ThemMoi.ViaHeShapefile frm = new QLHTDT.FormPhanHe.GiaoThong.ThemMoi.ViaHeShapefile();
            frm.Show();
            Cursor = Cursors.Default;
        }
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_TABLE_ViaHe_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Vỉa hè được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_ViaHe] " + ID1 + "";
                    ...
                    KienTruc.XoaDoiTuong("Vỉa hè", ID1);
                    KienTruc.dataAdapterNK.Update(KienTruc.TBNK);

                    MessageBox.Show("Xóa Vỉa hè thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Vỉa hè cần xóa", "Thông báo");
                }
            }
