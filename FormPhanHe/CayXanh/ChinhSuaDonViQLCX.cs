...
{
    public partial class ChinhSuaDonViQLCX : Form
    {
        DataTable tb;
        DataTable tbcheck;
        ...
        public ChinhSuaDonViQLCX()
        {
            InitializeComponent();
        }

        private void ChinhSuaDonViQLCX_Load(object sender, EventArgs e)
        {
            DataSet ds3 = new DataSet();
            SqlDataAdapter adp3 = new SqlDataAdapter("[PRC_QUERY_DonViQLCX_BY_ID] " + IDThietBi+"", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            adp3.Fill(ds3);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn sửa dữ liệu được chọn " + " không?", "Chỉnh sửa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string update = "[PRC_UPDATE_DonViQLCX] " + IDThietBi + ",N'" + txtTenDonVi.Text + "',N'" + txtMaDonVi.Text + "',N'" + txtGhiChu.Text + "'";
