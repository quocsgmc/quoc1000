...
{
    public partial class QuanLyCongVien : Form
    {
        ...
        string sql = "[PRC_QUERYTABLE_CONGVIEN]";
        public QuanLyCongVien()
        {
            InitializeComponent();
        ...
        private void QuanLyCongVien_Load(object sender, EventArgs e)
        {
        ...
        void showgridControl1()
        {
            ...
            {
                var val = ds.Tables[0].Rows[intCount]["BanQLDA"].ToString();
                if (!comboBanQLDAi.Items.Contains(val))
                {
                    comboBanQLDAi.Items.Add(val);
                }
            }
        }
        ...
        private void BtExcell_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa dữ liệu được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out IDDVCS);
                    string str = QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH);
                    SqlConnection conn = new SqlConnection(str);
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_CONGVIEN] " + IDDVCS + "";
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenCongVien"],
               new ColumnFilterInfo("[TenCongVien] like '%" + txtTenCongVien.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["BanQLDA"],
               new ColumnFilterInfo("[BanQLDA] like '%" + comboBanQLDAi.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            try
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out IDThietBi);
                QLHTDT.FormPhanHe.CayXanh.ChinhSuaCongVien frm = new QLHTDT.FormPhanHe.CayXanh.ChinhSuaCongVien();
                frm.Show();
                this.Close();
            }
            catch
        ...
        {
            txtTenCongVien.ResetText();
            comboBanQLDAi.ResetText();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            QLHTDT.FormPhanHe.CayXanh.ThemMoiCongVien frm = new QLHTDT.FormPhanHe.CayXanh.ThemMoiCongVien();
            frm.Show();
            this.Close();
