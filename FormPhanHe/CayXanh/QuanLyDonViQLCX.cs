...
{
    public partial class QuanLyDonViQLCX : Form
    {
        ...
        int IDDVCS;
        string sql = "[PRC_QUERYTABLE_DonViQLCX]";
        public QuanLyDonViQLCX()
        {
            InitializeComponent();
        ...
        private void QuanLyDonViQLCX_Load(object sender, EventArgs e)
        {
        ...
        private void BtExcell_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa dữ liệu được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDDVQLCX").ToString(), out IDDVCS);
                    string str = QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH);
                    SqlConnection conn = new SqlConnection(str);
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_DonViQLCX] " + IDDVCS + "";
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            try
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDDVQLCX").ToString(), out IDThietBi);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            QLHTDT.FormPhanHe.CayXanh.ThemMoiDonViQLCX frm = new QLHTDT.FormPhanHe.CayXanh.ThemMoiDonViQLCX();
            frm.Show();
            this.Close();
