...
{
    public partial class QuanLyTrangTri : Form
    {
        ...
        string sql = "PRC_QUERYTABLECayTrangTri";
        public static int ID1;
        public static int LoadLaiForm;
        string sql1 = "[PRC_QUERYCayTrangTri_BY_ID] " + ID1 + "";
        public QuanLyTrangTri()
        {
            InitializeComponent();
        ...
        private void QuanLyTrangTri_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.CayXanh.QLTrangTri.ChinhSuaMotTrangTri frm = new QLHTDT.FormPhanHe.CayXanh.QLTrangTri.ChinhSuaMotTrangTri();
                frm.ShowDialog(this);
                if (LoadLaiForm == 1)
                {
                    showgridControl1();
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Cây trang trí")
                    {
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Cây trang trí này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Cây trang trí \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Quản lý cây xanh\\Cây trang trí.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Cây trang trí")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Cây trang trí", "Thông báo"); }
                                else
                                {
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.CayXanh.QLTrangTri.NhatKyCSTrangTri.NhatKyCSTrangTri frm = new QLHTDT.FormPhanHe.CayXanh.QLTrangTri.NhatKyCSTrangTri.NhatKyCSTrangTri();
            frm.Show();
            Cursor = Cursors.Default;
        }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.CayXanh.QLTrangTri.ThemMoiMotTrangTri frm = new QLHTDT.FormPhanHe.CayXanh.QLTrangTri.ThemMoiMotTrangTri();
            frm.ShowDialog(this);
            if (LoadLaiForm == 1)
            {
                showgridControl1();
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.CayXanh.QLTrangTri.ThemMoiTuExcel frm = new QLHTDT.FormPhanHe.CayXanh.QLTrangTri.ThemMoiTuExcel();
            frm.ShowDialog(this);
            if (LoadLaiForm == 1)
            {
                showgridControl1();
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                    sql1 = "[PRC_QUERYCayTrangTri_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                    return;
                }
                catch
        ...
        private void comboBox1_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            switch (combobox1.Text)
            {
                ...
                case "Đường phố":
                    ...
                    gridColumn24.Width = 70;//Tên đường
                    gridColumn25.Width = 80; //Loại cây
                    gridColumn28.Width = 70; //Quận Huyện
                    break;
                case "Công viên":
                    ...
                    gridColumn2.Width = 70; //Tên công viên
                    gridColumn12.Width = 76;//Phường/Xã
                    gridColumn15.Width = 60;//Mã cây xanh
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Cây trang trí được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "OBJECTID").ToString(), out ID1);
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_CAYTRANGTRI_BY_ID] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Cây trang trí thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Cây trang trí cần xóa", "Thông báo");
                }
            }
        }
        ...
        private void button7_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                string path = Properties.Settings.Default.PathData + "\\Quản lý cây xanh\\Cây trang trí" + "\\" + GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "MaCay").ToString();
            ...
            {
                MessageBox.Show("Chưa chọn Cây xanh trang trí cần đính kèm file. Vui lòng thử lại", "Thông báo");
            }
        }
    }
}
