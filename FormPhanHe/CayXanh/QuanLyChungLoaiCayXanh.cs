...
{
    public partial class QuanLyChungLoaiCayXanh : Form
    {
        ...
        string sql = "[PRC_QUERYTABLE_ChungLoaiCayXanh]";
        public QuanLyChungLoaiCayXanh()
        {
            InitializeComponent();
        ...
        private void QuanLyChungLoaiCayXanh_Load(object sender, EventArgs e)
        {
        ...
        void showgridControl1()
        {
            ...
            {
                var val = ds.Tables[0].Rows[intCount]["PhanLoai"].ToString();
                if (!comboPhanLoai.Items.Contains(val))
                {
                    comboPhanLoai.Items.Add(val);
                }
            }
        }
        ...
        private void BtExcell_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa dữ liệu được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDChungLoaiCay").ToString(), out IDDVCS);
                    string str = QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH);
                    SqlConnection conn = new SqlConnection(str);
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_ChungLoaiCayXanh] " + IDDVCS + "";
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenDonVi"],
               new ColumnFilterInfo("[TenDonVi] like '%" + txtTenChungLoai.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["MaDonVi"],
               new ColumnFilterInfo("[MaDonVi] like '%" + comboPhanLoai.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            try
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDChungLoaiCay").ToString(), out IDThietBi);
                QLHTDT.FormPhanHe.CayXanh.ChinhSuaChungLoai frm = new QLHTDT.FormPhanHe.CayXanh.ChinhSuaChungLoai();
                frm.Show();
                this.Close();
            }
            catch
        ...
        {
            txtTenChungLoai.ResetText();
            comboPhanLoai.ResetText();
        ...
        private void txtIDLoaiTram_TextChanged(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenChungLoaiCay"],
              new ColumnFilterInfo("[TenChungLoaiCay] like '%" + txtTenChungLoai.Text + "%'", ""));
        }

        private void button2_Click(object sender, EventArgs e)
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.CayXanh.ThemMoiChungLoai frm = new QLHTDT.FormPhanHe.CayXanh.ThemMoiChungLoai();
            frm.Show();
            this.Close();
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["PhanLoai"],
               new ColumnFilterInfo("[PhanLoai] like '%" + comboPhanLoai.Text + "%'", ""));
        }
