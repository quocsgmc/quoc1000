...
{
    public partial class QuanLyLoaiChamSoc : Form
    {
        ...
        int IDDVCS;
        string sql = "[PRC_QUERYTABLE_LoaiChamSoc]";
        public QuanLyLoaiChamSoc()
        {
            InitializeComponent();
        ...
        private void QuanLyLoaiChamSoc_Load(object sender, EventArgs e)
        {
        ...
        void showgridControl1()
        {
            ...
            {
                var val = ds.Tables[0].Rows[intCount]["MaHieu"].ToString();
                if (!comboMaHieu.Items.Contains(val))
                {
                    comboMaHieu.Items.Add(val);
                }
            }
        }
        ...
        private void BtExcell_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa dữ liệu được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDLoaiChamSoc").ToString(), out IDDVCS);
                    string str = QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH);
                    SqlConnection conn = new SqlConnection(str);
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_LoaiChamSoc] " + IDDVCS + "";
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenDonVi"],
               new ColumnFilterInfo("[TenDonVi] like '%" + txtTenLoaiCS.Text + "%'", ""));
            view.ActiveFilter.Add(view.Columns["MaDonVi"],
               new ColumnFilterInfo("[MaDonVi] like '%" + comboMaHieu.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            try
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDLoaiChamSoc").ToString(), out IDThietBi);
                QLHTDT.FormPhanHe.CayXanh.ChinhSuaLoaiChamSoc frm = new QLHTDT.FormPhanHe.CayXanh.ChinhSuaLoaiChamSoc();
                frm.Show();
                this.Close();
            }
            catch
        ...
        {
            txtTenLoaiCS.ResetText();
            comboMaHieu.ResetText();
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["TenLoaiChamSoc"],
              new ColumnFilterInfo("[TenLoaiChamSoc] like '%" + txtTenLoaiCS.Text + "%'", ""));
        }

        private void button2_Click(object sender, EventArgs e)
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.CayXanh.ThemMoiLoaiChamSoc frm = new QLHTDT.FormPhanHe.CayXanh.ThemMoiLoaiChamSoc();
            frm.ShowDialog();
            this.Close();
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["MaHieu"],
               new ColumnFilterInfo("[MaHieu] like '%" + comboMaHieu.Text + "%'", ""));
        }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.CayXanh.UploadLoaiChamSoc frm = new QLHTDT.FormPhanHe.CayXanh.UploadLoaiChamSoc();
            frm.Show();
            this.Close();
            Cursor = Cursors.Default;
        }
