...
{
    public partial class UpdateToaDoThuThap : Form
    {
        DataTable dt = new DataTable();
        public UpdateToaDoThuThap()
        {
            InitializeComponent();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    for (iLoi = 0; iLoi < dgvData.RowCount - 1; iLoi++)
                    {
                        ...
                        if (comboBox2.Text != "" )
                        { ToaDoX = dgvData.Rows[iLoi].Cells[ToaDoX1].Value.ToString(); }
                        else { ToaDoX = "null"; }
                        if (comboBox3.Text != "")
                        { ToaDoY = dgvData.Rows[iLoi].Cells[ToaDoY1].Value.ToString(); }
                        else { ToaDoY = ""; }
                        
                        //cập nhật thuộc tính đối tượng
                        SqlConnection conn = new SqlConnection(QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
                        conn.Open();
                        string sql1 = "[PRC_UPDATE_CAYXANH_THUTHAP] " //update Cây xanh
                       + " N'" + IDCayXanh1
                       + "', " + ToaDoX
                       + ", " + ToaDoY
                        + ",'Point(" + ToaDoX + " " + ToaDoY + ")'";
                    ...
                    MessageBox.Show("Cập nhật tọa độ thành công", "Thông báo");
            ...
            catch
            {
                ...
                MessageBox.Show("Cập nhật tọa độ thất bại. Vui lòng kiểm tra dữ liệu", "Thông báo");
                Cursor = Cursors.Default;
            }
