...
{
    public partial class QuanLyDapDang : Form
    {
        ...
        private void QuanLyDapDang_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCT").ToString(), out ID1);
                QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaDapDang frm = new QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaDapDang();
                frm.Show();
            }
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCT").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_TABLE_DapDang_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Đập dâng được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCT").ToString(), out ID1);
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_DapDang] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Đập dâng thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Đập dâng cần xóa", "Thông báo");
                }
            }
