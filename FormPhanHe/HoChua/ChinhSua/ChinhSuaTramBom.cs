...
{
    public partial class ChinhSuaTramBom : Form
    {
        ...
        int IDChinhSua = QLHTDT.FormPhanHe.HoChua.QuanlyTramBom.QuanlyTramBom.ID1;
        public ChinhSuaTramBom()
        {
            InitializeComponent();
            if (QLHTDT.FormChinh.KienTruc.FeatureWorkspace != null)
            { featureWorkspaceSDE = QLHTDT.FormChinh.KienTruc.FeatureWorkspace; }
        }
        private void ChinhSuaTramBom_Load(object sender, EventArgs e)
        {
            ...
            if (IDChinhSua != 0)
            {
                SqlDataAdapter adp3 = new SqlDataAdapter("[PRC_QUERY_TABLE_TramBom_BY_ID] " + IDChinhSua + "", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
                DataSet ds3 = new DataSet();
                adp3.Fill(ds3);
                textbox01.Text = ds3.Tables[0].Rows[0]["TenCongTrinh"].ToString();
                textbox02.Text = ds3.Tables[0].Rows[0]["PhanLoai"].ToString();
                textbox03.Text = ds3.Tables[0].Rows[0]["QuyMo"].ToString();
                textbox04.Text = ds3.Tables[0].Rows[0]["GhiChu"].ToString();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (check != false)
            {
                try
                {
                        ...
                        conn.Open();
                        string sql1 = "[PRC_UPDATE_TRAMBOM] "
                           + " " + objectid
                           + ", N'" + textbox01.Text
                           + "', N'" + textbox02.Text
                           + "', N'" + textbox03.Text
                        ...
                        MessageBox.Show("Chỉnh sửa Trạm bơm thành công", "Thông báo");
                ...
                catch
                {
                    ...
                    MessageBox.Show("Chỉnh sửa Trạm bơm thất bại. Vui lòng kiểm tra dữ liệu", "Thông báo");
