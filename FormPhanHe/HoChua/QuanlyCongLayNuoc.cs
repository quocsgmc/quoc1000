...
namespace QLHTDT.FormPhanHe.HoChua.QuanlyCongLayNuoc
{
    public partial class QuanlyCongLayNuoc : Form
    {
        ...
        private void QuanlyCongLayNuoc_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter adp3 = new SqlDataAdapter("select TenCongTrinh from HOCHUA", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            adp3.Fill(ds3);
            comboHoChua.Items.Clear();
            comboHoChua.Items.Add("");
            for (int intCount = 0; intCount < ds3.Tables[0].Rows.Count; intCount++)
            {
                var val = ds3.Tables[0].Rows[intCount]["TenCongTrinh"].ToString();

                if (!comboHoChua.Items.Contains(val))
                {
                    comboHoChua.Items.Add(val);
                }
            }
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenCongTrinh"],
               new ColumnFilterInfo("[TenCongTrinh] like '%" + comboHoChua.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCong").ToString(), out ID1);
                QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaCongLayNuoc frm = new QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaCongLayNuoc();
                frm.Show();
            }
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenChuDauTu"],
               new ColumnFilterInfo("[TenChuDauTu] like '%" + comboHoChua.Text + "%'", ""));
        }
        int MoRong = 1;
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCong").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_TABLE_CongLayNuoc_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa Cống lấy nước được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_CongLayNuoc] " + ID1 + "";
