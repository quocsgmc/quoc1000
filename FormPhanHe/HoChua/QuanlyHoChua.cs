...
namespace QLHTDT.FormPhanHe.HoChua.QuanlyHoChua
{
    public partial class QuanlyHoChua : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_HoChua]";
        public static int ID1 ;
        public static int IDKenhChinh;
        public static int IDDapHoChua ;
        public static int IDTran;
        public static int IDCongLayNuoc;
        public static int LoadLaiForm = 0;

        public static string MaHuyen = "null";
        string sql1 = "[PRC_QUERY_TABLE_HoChua_BY_ID] " + ID1 + "";
        ...
        private void QuanlyHoChua_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter adp1 = new SqlDataAdapter("Select TenCongTrinh from HOCHUA", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            DataSet ds1 = new DataSet();
            adp1.Fill(ds1);
            for (int intCount = 0; intCount < ds1.Tables[0].Rows.Count; intCount++)
            {
                var val2 = ds1.Tables[0].Rows[intCount]["TenCongTrinh"].ToString();
                if (!comboTenCT.Items.Contains(val2))
                {
                    comboTenCT.Items.Add(val2);
                }
            }
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenCongTrinh"],
               new ColumnFilterInfo("[TenCongTrinh] like '%" + comboTenCT.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out ID1);
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDKenh").ToString(), out IDKenhChinh);
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDTran").ToString(), out IDTran);
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDDap").ToString(), out IDDapHoChua);
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDCong").ToString(), out IDCongLayNuoc);
                QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaTongHop frm = new QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaTongHop();
                frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Hồ chứa")
                    {
                        IFeatureLayer ilayer1 = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1) as IFeatureLayer;
                        KTMoLop = KTMoLop + 1;
                        int ID;
                        int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out ID);
                        if (ID == 0)
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian Hồ chứa này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Hồ chứa \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + "\\Hồ chứa\\Hồ chứa.lyr");
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Hồ chứa")
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int ID;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Hồ chứa", "Thông báo"); }
                                else
                                {
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_TABLE_HoChua_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Hồ chứa được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out ID1);
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_HoChua] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Hồ chứa thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Hồ chứa cần xóa", "Thông báo");
                }
            }
        ...
        {
            if (vGridControl1.FocusedRow.Name.ToString() == "TTDapHoChua")
            {
                int IDHo;
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out IDHo);
                DoRowDoubleClick_Layer("Đập hồ chứa", IDHo, "IDDap", "\\Hồ chứa\\Đập hồ chứa.lyr");
            }
            if (vGridControl1.FocusedRow.Name.ToString() == "TTKenhChinh")
            {
                int IDHo;
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out IDHo);
                DoRowDoubleClick_Layer("Kênh chính", IDHo, "IDKenh", "\\Hồ chứa\\Kênh chính.lyr");
            }
            if (vGridControl1.FocusedRow.Name.ToString() == "TTTran")
            {
                int IDHo;
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out IDHo);
                DoRowDoubleClick_Layer("Tràn", IDHo, "IDTran", "\\Hồ chứa\\Tràn.lyr");
            }
            if (vGridControl1.FocusedRow.Name.ToString() == "TTCongLayNuoc")
            {
                int IDHo;
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "IDHo").ToString(), out IDHo);
                DoRowDoubleClick_Layer("Cống lấy nước", IDHo, "IDCong", "\\Hồ chứa\\Cống lấy nước.lyr");
            }
        }
        private void DoRowDoubleClick_Layer(string TenLop, int ID,string Objectid, string DuongDan)
        {
            Cursor = Cursors.WaitCursor;
            int KTMoLop = 0;
            string PhuongKT = null;
            try
            {
                ...
                for (int i1 = 0; i1 < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i1++)
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == TenLop)
                    {
                        IFeatureLayer ilayer1 = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1) as IFeatureLayer;
                        KTMoLop = KTMoLop + 1;
                        ...
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian "+TenLop+" này", "Thông báo");
                        }
                        else
                        {
                            IFeature ife = ilayer1.FeatureClass.GetFeature(ID);
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian "+TenLop+" \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        Cursor = Cursors.WaitCursor;
                        QLHTDT.FormChinh.KienTruc.axMapControl1.AddLayerFromFile(QLHTDT.Properties.Settings.Default.PathData + DuongDan);
                        QLHTDT.FormChinh.KienTruc.axMapControl1.Refresh();
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == TenLop)
                            {
                                KTMoLop = KTMoLop + 1;
                                IFeatureLayer ilayer = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i) as IFeatureLayer;
                                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, Objectid).ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian "+TenLop+"", "Thông báo"); }
                                else
                                {
                                    IFeature ife = ilayer.FeatureClass.GetFeature(ID);
                                    if (ife != null)
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.HoChua.ThemMoi.ThemMoiHoChua frm = new QLHTDT.FormPhanHe.HoChua.ThemMoi.ThemMoiHoChua();
            frm.ShowDialog(this);
            if (LoadLaiForm == 1)
            {
                showgridControl1();
