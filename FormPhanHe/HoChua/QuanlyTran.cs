...
{
    public partial class QuanlyTran : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_Tran]";
        public static int ID1 ;
        public static int LoadLaiForm = 0;

        public static string MaHuyen = "null";
        string sql1 = "[PRC_QUERY_TABLE_Tran_BY_ID] " + ID1 + "";
        ...
        public QuanlyTran(int IDHo)
        {
            InitializeComponent();
            dmap = QLHTDT.FormChinh.KienTruc.axMapControl1.Map;
            sql = "PRC_QUERY_TABLE_Tran " + IDHo + "";
        }
        

        private void QuanlyTran_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            {
                int.TryParse(cardView1.GetRowCellValue(cardView1.FocusedRowHandle, "IDTran").ToString(), out ID1);
                //QLHTDT.FormPhanHe.HoChua.ChinhSua.Chi frm = new QLHTDT.FormPhanHe.BuuChinh_VienThong.TramBTS.ChinhSuaMotTram();
                //frm.Show();
            }
        ...
        private void DoRowDoubleClick(GridView view, System.Drawing.Point pt)
        {
            ...
            try
            {
                ...
                {
                    if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1).Name == "Tràn")
                    {
                        IFeatureLayer ilayer1 = QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i1) as IFeatureLayer;
                        KTMoLop = KTMoLop + 1;
                        int ID;
                        int.TryParse(cardView1.GetRowCellValue(cardView1.FocusedRowHandle, "IDTran").ToString(), out ID);
                        if (ID == 0)
                        {
                            Cursor = Cursors.WaitCursor;
                            MessageBox.Show("Không có dữ liệu không gian IDTran này", "Thông báo");
                        }
                        else
                        {
                ...
                if (KTMoLop == 0)
                {
                    DialogResult dialogResult = MessageBox.Show("Chưa mở lớp dữ liệu không gian Tràn \n" + "Có muốn mở lớp dữ liệu này hay không ?", "Thông báo", MessageBoxButtons.YesNo);
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        for (int i = 0; i < QLHTDT.FormChinh.KienTruc.axMapControl1.LayerCount; i++)
                        {
                            if (QLHTDT.FormChinh.KienTruc.axMapControl1.get_Layer(i).Name == "Trạm BTS")
                            {
                                ...
                                int ID;
                                int.TryParse(cardView1.GetRowCellValue(cardView1.FocusedRowHandle, "IDTran").ToString(), out ID);
                                if (ID == 0) { MessageBox.Show("Không có dữ liệu không gian Tràn", "Thông báo"); }
                                else
                                {
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Tràn được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(cardView1.GetRowCellValue(cardView1.FocusedRowHandle, "IDTran").ToString(), out ID1);
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_Tran] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Tràn thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Tràn cần xóa", "Thông báo");
                }
            }
