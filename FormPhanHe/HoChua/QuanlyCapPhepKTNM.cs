...
{
    public partial class QuanlyCapPhepKTNM : Form
    {
        ...
        string sql = "[PRC_QUERY_TABLE_CapPhepKTNM]";
        ...
        private void QuanlyCapPhepKTNM_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter adp1 = new SqlDataAdapter("select DonViXinCP from GPKhaiThacNuocMat", QLHTDT.FormPhu.QTHT.MaHoa.Rot13(QLHTDT.Properties.Settings.Default.strConnectionDAQH));
            DataSet ds1 = new DataSet();
            adp1.Fill(ds1);
            comboTenDonVi.Items.Clear();
            comboTenDonVi.Items.Add("");
            for (int intCount = 0; intCount < ds1.Tables[0].Rows.Count; intCount++)
            {
                var val = ds1.Tables[0].Rows[intCount]["DonViXinCP"].ToString();

                if (!comboTenDonVi.Items.Contains(val))
                {
                    comboTenDonVi.Items.Add(val);
                }
            }
        ...
        private void BtTracuu_Click(object sender, EventArgs e)
        {
            ...
            view.ActiveFilter.Add(view.Columns["TenCongTrinh"],
               new ColumnFilterInfo("[TenCongTrinh] like '%" + comboTenDonVi.Text + "%'", ""));
        }
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaCapPhepKTNM frm = new QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaCapPhepKTNM();
                frm.Show();
            }
        ...
        {
            Cursor = Cursors.WaitCursor;
            QLHTDT.FormPhanHe.HoChua.ThemMoi.ThemMoiCapPhepKTMN frm = new QLHTDT.FormPhanHe.HoChua.ThemMoi.ThemMoiCapPhepKTMN();
            frm.ShowDialog(this);
            if (LoadLaiForm == 1)
            {
                showgridControl1();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            Cursor = Cursors.WaitCursor;
            int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString(), out ID1);
            QLHTDT.FormPhanHe.HoChua.QuanlyCTKTNM.QuanlyCTKTNM frm = new QLHTDT.FormPhanHe.HoChua.QuanlyCTKTNM.QuanlyCTKTNM(ID1);
            frm.ShowDialog(this);
            Cursor = Cursors.Default;
        }
        ...
        {
            ColumnView view = GridView1;
            view.ActiveFilter.Add(view.Columns["DonViXinCP"],
               new ColumnFilterInfo("[DonViXinCP] like '%" + comboTenDonVi.Text + "%'", ""));
        }
        int MoRong = 1;
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa Cấp phép được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_CapPhepKTNM] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Cấp phép thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Cấp phép cần xóa", "Thông báo");
                }
            }
