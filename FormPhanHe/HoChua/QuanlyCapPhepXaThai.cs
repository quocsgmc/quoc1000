...
{
    public partial class QuanlyCapPhepXaThai : Form
    {
        ...
        private void QuanlyCapPhepXaThai_Load(object sender, EventArgs e)
        {
        ...
        public void ChinhSua_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaCapPhepXaThai frm = new QLHTDT.FormPhanHe.HoChua.ChinhSua.ChinhSuaCapPhepXaThai();
                frm.Show();
            }
        ...
        private void GridView1_Click(object sender, EventArgs e)
        {
            if (int.Parse(GridView1.FocusedRowHandle.ToString()) >= 0)
            {
                ...
                {
                    int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString(), out ID1);
                    sql1 = "[PRC_QUERY_TABLE_CapPhepXaThai_BY_ID] " + ID1 + "";
                    showgridControl2(sql1);
                }
                catch
                {
        ...
        {
            if (MessageBox.Show("Bạn muốn xóa Giấy phép được chọn " + " không?", "Xóa dữ liệu", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int.TryParse(GridView1.GetRowCellValue(GridView1.FocusedRowHandle, "ID").ToString(), out ID1);
                try
                {
                    ...
                    conn.Open();
                    string Delelequery = "[PRC_DELETE_CapPhepXaThai] " + ID1 + "";
                    SqlCommand cmd1 = new SqlCommand(Delelequery, conn);
                    cmd1.ExecuteNonQuery();
                    showgridControl1();
                    MessageBox.Show("Xóa Giấy phép thành công", "Thông báo");
                }
                catch
                {
                    MessageBox.Show("Vui lòng chọn Giấy phép cần xóa", "Thông báo");
                }
            }
